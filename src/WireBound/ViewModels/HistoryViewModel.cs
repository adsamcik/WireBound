...
public sealed partial class HistoryViewModel : ObservableObject, IDisposable
{
    ...
    [ObservableProperty]
    public partial ObservableCollection<DailyUsageItem> DailyUsages { get; set; }
    ...
    [
        ...
        {
            Name = "Date",
            NamePaint = new SolidColorPaint(ChartColors.AxisLabelColor),
    ...
    private async void InitializeAsync()
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"HistoryViewModel.LoadDataAsync failed: {ex.Message}");
        }
    }
    ...
    {
        var dailyData = await _persistence.GetDailyUsageAsync(StartDate, EndDate).ConfigureAwait(false);
        var (totalReceived, totalSent) = await _persistence.GetTotalUsageAsync().ConfigureAwait(false);
        ...
        MainThread.BeginInvokeOnMainThread(() =>
        {
            ...
            {
                DailyUsages.Add(new DailyUsageItem(usage));
            }
